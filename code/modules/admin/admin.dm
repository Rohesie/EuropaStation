	...
	switch(admincaster_screen)
		...
		if(9)
			...
			if(src.admincaster_feed_channel.censored)
				dat+={"
					<FONT COLOR='red'><B>ATTENTION: </B></FONT>This channel has been deemed as threatening to the welfare of the crew, and marked with a [using_map.company_name] D-Notice.<BR>
		...
		if(11)
			dat+={"
				...
				<FONT SIZE=1>A D-Notice is to be bestowed upon the channel if the handling Authority deems it as harmful for the crew's
		...
		if(13)
			dat+={"
				<B>[src.admincaster_feed_channel.channel_name]: </B><FONT SIZE=1>\[ created by: <FONT COLOR='maroon'>[src.admincaster_feed_channel.author]</FONT> \]</FONT><BR>
				Channel messages listed below. If you deem them dangerous to the crew, you can <A href='?src=\ref[src];ac_toggle_d_notice=\ref[src.admincaster_feed_channel]'>Bestow a D-Notice upon the channel</A>.<HR>
			"}
			if(src.admincaster_feed_channel.censored)
				dat+={"
					<FONT COLOR='red'><B>ATTENTION: </B></FONT>This channel has been deemed as threatening to the welfare of the crew, and marked with a [using_map.company_name] D-Notice.<BR>
		...
		if(18)
			dat+={"
				<B><FONT COLOR ='maroon'>-- PRIORITY WANTED ISSUE --</B></FONT><BR><FONT SIZE=2>\[Submitted by: <FONT COLOR='green'>[news_network.wanted_issue.backup_author]</FONT>\]</FONT><HR>
...
/datum/admins/proc/Game()
	...
	dat += {"
		...
		<A href='?src=\ref[src];vsc=phoron'>Edit Toxins Settings</A><br>
...
/datum/admins/proc/show_game_mode()
	...
	if(length(ticker.mode.antag_templates))
		for(var/datum/antagonist/antag in ticker.mode.antag_templates)
